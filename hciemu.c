/*
 *
 *  Bluetooth device emulation - HCIemu
 *
 ...
 *
 *
 *  $Id: hciemu.c,v 1.15 2005/07/05 17:34:24 holtmann Exp $
 */
...
static int run_proxy(int fd, int dev)
{
...
int main(int argc, char *argv[], char *env[])
{
	...
	GIOChannel *dev_io;
	char *device = NULL;
	int fd, opt, daemon, dofork, dev = -1;

	/* Configure default settings */
	daemon = 1; dofork = 1;

	while ((opt=getopt_long(argc, argv, "d:nh", main_options, NULL)) != EOF) {
		switch(opt) {
		case 'd':
